...
const StaffDashboard = () => {
  ...
  return (
    <DashboardLayout title={`Khu vực Nhân viên: ${user ? user.name : 'Staff Portal'}`} menuItems={menuItems} activeTab={activeTab} setActiveTab={setActiveTab}>
      ...
      {/* Live Chat Handoff Box - Cảnh báo & Trả lời trực tiếp */}
      {(needsHumanAssistance || isHandoffActive) && (
        <div style={{ 
          ...
          borderRadius: '16px', 
          marginBottom: '2rem',
          boxShadow: '0 8px 24px rgba(251, 140, 0, 0.2)',
          overflow: 'hidden'
        }}>
          {/* Header Báo động */}
          <div style={{ background: 'linear-gradient(135deg, #E65100 0%, #F57C00 100%)', color: 'white', padding: '1.2rem 1.8rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span>🚨</span> BÁO ĐỘNG HỖ TRỢ TRỰC TUYẾN: KHÁCH HÀNG ĐANG ĐỜI TRẢ LỜI
              </h3>
              <p style={{ margin: '4px 0 0 0', fontSize: '0.9rem', opacity: 0.9 }}>
                Khách hàng <strong>{handoffCustomerName}</strong> đang yêu cầu gặp trực tiếp Chuyên viên tư vấn trên Chatbot AI Bé Mèo!
              </p>
            </div>
            <button 
              onClick={() => { resolveHandoff(); endHandoff(); }}
              style={{ background: 'white', color: '#E65100', border: 'none', padding: '0.5rem 1.2rem', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 700, cursor: 'pointer' }}
            >
              ✅ Hoàn thành tư vấn
            </button>
          </div>
          ...
          {/* Live Chat Content */}
          <div style={{ padding: '1.5rem', background: '#FFFDF9' }}>
            <div style={{ fontSize: '0.9rem', fontWeight: 600, color: '#E65100', marginBottom: '0.8rem' }}>
              💬 NỘI DUNG TRAO ĐỔI TRỰC TIẾP VỚI KHÁCH HÀNG:
            </div>

            <div style={{ maxHeight: '250px', overflowY: 'auto', background: 'white', border: '1px solid #FFE0B2', borderRadius: '12px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', marginBottom: '1rem' }}>
              {liveChatMessages.length === 0 ? (
                <div className="text-muted text-center" style={{ padding: '1rem' }}>Chưa có tin nhắn nào. Đang chờ khách hàng nhập câu hỏi...</div>
              ) : (
                liveChatMessages.map(msg => (
                  <div key={msg.id} style={{ display: 'flex', justifyContent: msg.sender === 'staff' ? 'flex-end' : 'flex-start' }}>
                    <div style={{
                      ...
                      borderRadius: '14px',
                      background: msg.sender === 'staff' ? 'var(--color-secondary)' : '#FFF3E0',
                      color: msg.sender === 'staff' ? 'white' : '#333',
                      fontSize: '0.92rem',
                      lineHeight: 1.5,
                      border: msg.sender === 'customer' ? '1px solid #FFE0B2' : 'none'
                    }}>
                      <div style={{ fontSize: '0.78rem', opacity: 0.8, marginBottom: '2px', fontWeight: 600 }}>
                        {msg.sender === 'customer' ? `👤 ${msg.senderName}` : `👩‍🦰 ${msg.senderName}`} • {msg.time}
                      </div>
                      {msg.text}
                    </div>
                  </div>
            ...
            {/* Input Form gửi phản hồi cho khách */}
            <form onSubmit={handleSendStaffReply} style={{ display: 'flex', gap: '0.8rem' }}>
              <input 
                type="text" 
                className="form-input" 
                value={staffReplyInput}
                onChange={e => setStaffReplyInput(e.target.value)}
                placeholder="Nhập câu trả lời tư vấn gửi trực tiếp sang ô chat của khách hàng..." 
                style={{ flex: 1, padding: '0.8rem 1.2rem', border: '1px solid #FB8C00', borderRadius: '25px', outline: 'none' }}
              />
              <button 
                type="submit" 
                className="btn" 
                style={{ background: '#E65100', color: 'white', fontWeight: 700, padding: '0.8rem 1.5rem', borderRadius: '25px', display: 'flex', alignItems: 'center', gap: '6px' }}
              >
                🚀 Gửi cho khách
              </button>
            </form>
